...
bool CmdExpression::Process(Parser& parser) {
    ...
    const std::string left(parser.get_lexem().name);
    if (parser.get_lexem().type != LT_Delimiter ||
        parser.get_last().delimiter != '=')
        throw "Bad assigment";

    const float value = evaluate_expression(parser);
...
struct ForInfo {
    ...
    ForInfo() : upper_limit(0), step(1.0f) {}
};
...
bool CmdNext::Process(Parser& parser) {
    ...
    const ForInfo for_info = CmdForStack.top();
...
bool CmdIf::Process(Parser& parser) {
    ...
    const bool cond = (evaluate_expression(parser) != 0.0f);
